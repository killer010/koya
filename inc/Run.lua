...
koya = Token:match("(%d+)")
redis:set(koya..":VERSION",1)
redis:set(koya..":SUDO_ID:",SUDO_USER)
redis:set(koya..":DataCenter:",'German')
redis:set(koya..":UserNameBot:",BOT_User)
redis:set(koya..":NameBot:",BOT_NAME)
redis:hset(koya..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3koya_INSTALL","Yes")
info = {} 
...
koya = Token:match("(%d+)")
our_id = tonumber(koya)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(koya..":UserNameBot:")
SUDO_ID = tonumber(redis:get(koya..":SUDO_ID:"))
SUDO_USER = redis:hgetall(koya..'username:'..SUDO_ID).username
version = redis:get(koya..":VERSION")
DataCenter = redis:get(koya..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..koya..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(koya..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(koya..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(koya..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(koya..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(koya..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(koya..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(koya..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(koya..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(koya..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(koya..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(koya..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(koya..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(koya..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(koya..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(koya..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
قناة سورس كويا 🇪🇬@KOYA_SOURCE
...
if msg.GroupActive and redis:get(koya..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(koya..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(koya..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(koya..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.koya then 
if msg.text and ScriptFile.ikoya then
for k, koya in pairs(ScriptFile.koya) do
local SearchText = msg.text:match(koya)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ikoya(msg,{msg.text:match(koya)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",koya.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dkoya then
if ScriptFile.dkoya(msg) == false then
return false
end
...
if Plug.koya then 
if msg.text and Plug.ikoya then
for k, koya in pairs(Plug.koya) do
local SearchText = msg.text:match(koya)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(koya..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(koya..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ikoya(msg,{msg.text:match(koya)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",koya.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source koya")
end 

end
else
print("The File Script.lua Not Runing in The Source koya")
...
function tdcli_update_callback(data)
	...
	redis:srem(koya..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRcekoya/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRcekoya/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRcekoya/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRcekoya/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRcekoya/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRcekoya/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(koya..":SUDO_ID:")
redis:hset(koya..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(koya..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(koya..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(koya..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(koya..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(koya..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(koya..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(koya..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(koya..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(koya..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(koya..'group:add'..data.chat_.id_) then
	redis:set(koya..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(koya..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(koya..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(koya..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	koya..'group:name'..k,IDS.Title,
	koya..'num_msg_koya'..k,5,
	koya..'group:add'..k,true,
	koya..'lock_link'..k,true,
	koya..'lock_spam'..k,true,
	koya..'lock_webpage'..k,true,
	koya..'lock_markdown'..k,true,
	koya..'lock_flood'..k,true,
	koya..'lock_bots'..k,true,
	koya..'mute_forward'..k,true,
	koya..'mute_contact'..k,true,
	koya..'mute_document'..k,true,
	koya..'mute_inline'..k,true,
	koya..'lock_username'..k,true,
	koya..'replay'..k,true
	)
	redis:sadd(koya..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(koya..'username:'..ID,'username',user)
	redis:sadd(koya..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(koya..'username:'..ID,'username',user)
	redis:sadd(koya..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(koya..'username:'..ID,'username',user)
	redis:sadd(koya..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(koya..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(koya..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(koya..'username:'..data.user_.id_).username
	...
	redis:hset(koya..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
